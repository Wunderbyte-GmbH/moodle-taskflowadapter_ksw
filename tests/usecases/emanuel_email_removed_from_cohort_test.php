...
final class emanuel_email_removed_from_cohort_test extends advanced_testcase {
    ...
    protected function set_messages_db(): array {
        global $DB;
        $messageids = [];
        $messages = json_decode(file_get_contents(__DIR__ . '/../mock/messages/register_messages.json'));
    ...
    public function test_betty_best(): void {
        ...
        // Add to cohort.
        cohort_add_member($cohort->id, $user->id);
        $scheduledtasks = $DB->get_records('task_adhoc');
        $this->assertNotEmpty($scheduledtasks);

        $this->runAdhocTasks();
        $droppendinassignment = $DB->get_records('local_taskflow_assignment');
        foreach ($droppendinassignment as $assignment) {
            $this->assertNotNull($assignment->duedate);
        }
        $sentmessages = $DB->get_records('local_taskflow_sent_messages');
