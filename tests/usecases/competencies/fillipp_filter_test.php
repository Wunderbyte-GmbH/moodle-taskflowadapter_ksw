 ...
 *
 */
final class fillipp_filter_test extends advanced_testcase {
    ...
    /**
     * Test variant: change rule filter to exclude/include user instead of changing cohort/unit.
     * Intentionally asserts reassignment after inclusive filter update (expected to fail currently).
     * @covers \local_taskflow\local\completion_process\completion_operator
     *
     * @param array $bdata
     * @dataProvider booking_common_settings_provider
     */
    public function test_filipp_filter_change($bdata): void {
        global $DB;
        $sink = $this->redirectEmails();
        $lock = $this->createMock(\core\lock\lock::class);
        $cronlock = $this->createMock(\core\lock\lock::class);
        ...
        // Ensure assignments and messages start clean.
        $plugingeneratortf = self::getDataGenerator()->get_plugin_generator('local_taskflow');
        $plugingeneratortf->runtaskswithintime($cronlock, $lock, time());

        // Ensure there is an active assignment for Chris before changing filters.
        // When we start, there is an assignment for Chris from the initial setup.
        $userchris = $DB->get_record('user', ['firstname' => 'Chris']);
        ...
        $this->assertSame((int)$activeassignmentprechange->ruleid, $rule['id']);
        $this->assertSame((int)$activeassignmentprechange->status, assignment_status_facade::get_status_identifier('assigned'));

        // Prepare: add an inclusive filter to the first rule (Org2 equals empty string).
        $rulejson = json_decode($rule['rulejson'], true);
        $oldfilter = $rulejson['rulejson']['rule']['filter'] ?? [];
        $rulejson['rulejson']['rule']['filter'] = [
            [
                'filtertype' => 'user_profile_field',
                'userprofilefield' => 'filtertestfield',
                'operator' => 'equals',
                'value' => 'a',
            ],
        ];
        ...
        // Trigger rule update and run tasks.
        $event = rule_created_updated::create([
            'objectid' => $rule['id'],
            'context'  => context_system::instance(),
            'other'    => [
        ...
        $event->trigger();
        $plugingeneratortf->runtaskswithintime($cronlock, $lock, time());
        // Advance time to allow assignments to be scheduled and messages to be sent.
        time_mock::set_mock_time(strtotime('+ 6 minutes', time()));
        $plugingeneratortf->runtaskswithintime($cronlock, $lock, time());

        // Chris should still have an active assignment for the first rule.
        $assignmentsafterruleupdate = $DB->get_records('local_taskflow_assignment', ['userid' => $userchris->id]);
        $this->assertCount(1, $assignmentsafterruleupdate);
        $assignactive = array_pop($assignmentsafterruleupdate);
        $this->assertSame((int)$assignactive->ruleid, $rule['id']);
        $this->assertSame((int)$assignactive->status, assignment_status_facade::get_status_identifier('droppedout'));
        $this->assertSame((int)$assignactive->active, 0);

        // Assignment is not active anymore.
        // Next steps.
        // - take filter out again -> status should come back.
        ...
        rules::destroy_instance();
        // Trigger rule update and run tasks -> assignment should become droppedout.
        $event = rule_created_updated::create([
            'objectid' => $rule['id'],
            'context'  => context_system::instance(),
            'other'    => [
        ...
        $assignmentspostchange = $DB->get_records('local_taskflow_assignment', ['userid' => $userchris->id]);
        $this->assertCount(1, $assignmentspostchange);
        $assignmentpostchange = array_pop($assignmentspostchange);
        $this->assertSame((int)$assignmentpostchange->ruleid, $rule['id']);
        $this->assertSame((int)$assignmentpostchange->status, assignment_status_facade::get_status_identifier('assigned'));
        $this->assertSame((int)$assignmentpostchange->active, 1);

        // Change filter back to include Chris (Org2 equals empty).
        $rulejson['rulejson']['rule']['filter'] = [
            [
                'filtertype' => 'user_profile_field',
                'userprofilefield' => 'filtertestfield',
        ...
        // Now update user to match filter.
        global $CFG;
        require_once($CFG->dirroot . '/user/profile/lib.php');
        $userchris->profile_field_filtertestfield = 'a';
        profile_save_data($userchris);

        \core\event\user_updated::create_from_userid($userchris->id)->trigger();
        ...
        $assignactive = array_pop($assignmentsafterruleupdate);
        $this->assertSame((int)$assignactive->ruleid, $rule['id']);
        $this->assertSame((int)$assignactive->status, assignment_status_facade::get_status_identifier('assigned'));
        $this->assertSame((int)$assignactive->active, 1);
    }
}
