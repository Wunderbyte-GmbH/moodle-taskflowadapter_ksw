...
final class betty_best_test extends advanced_testcase {
     ...
     * @dataProvider booking_common_settings_provider
     */
    public function test_assign_competency_on_course_completion_in_hierarchy_multi_overdue(array $bdata): void {
        global $DB, $CFG;
        ...
        $assignment = $DB->get_record('local_taskflow_assignment', ['id' => $assignment->id ]);
        ...
        // + booking course enrol2 + completed status + comp1 completed + comp2 completed.
        $countafter = count($assignmenthistory = $DB->get_records('local_taskflow_history', ['assignmentid' => $assignment->id]));
        $this->assertSame($initialcount + 9, $countafter);
        ...
        // Should be assigned + assigned msg.
        $countafter = count($assignmenthistory = $DB->get_records('local_taskflow_history', ['assignmentid' => $assignment->id]));
        $this->assertSame($initialcount + 11, $countafter);
        ...
        $this->assertCount(4, $sentmessages);
        $this->assertCount(4, $messagesinkuser2);
